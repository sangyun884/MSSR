...
#     def __init__(self, opt, n_res_blocks=8, scale=4):
...
#         self.noises = nn.ModuleList([NoiseInjection(64, opt.noise_std) for _ in range(n_res_blocks)])
#         if self.scale == 4:
...
#         for res_block, noise in zip(self.res_blocks, self.noises):
#             if self.opt.use_noise: block = noise(block)
#             block = res_block(block)
...
class De_resnet(nn.Module):
    def __init__(self, opt, n_res_blocks=8, scale=4):
        ...
        if opt.use_noise:
            ...
            self.noises = [None for _ in range(n_res_blocks)]
        if self.scale == 4:
            self.down_sample = nn.Sequential(
                nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1),
                nn.PReLU(),
    ...
    def forward(self, x):
        ...
        block = self.block_output(block)
        return torch.sigmoid(block)
    # def set_std(self,std, start, end):

    #     for i,n in enumerate(self.noises[start:end]):
    #         n.set_std(std)
    #         print(i, "setted / ", len(self.noises))
