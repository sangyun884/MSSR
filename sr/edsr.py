...
class EDSR(nn.Module):
    def __init__(self, conv=default_conv, synchronize_norm = False, gpu=0, scale_factor = 4):
    ...
    def forward(self, x):
        if self.synchronize_norm:
            ...
            x = x*0.5
            x = x + torch.tensor([[[0.0512]], [[0.0629]], [[0.096]]]).cuda(self.gpu)
            
        else:
            # When input is in [0,1]
            x = x - torch.tensor([[[0.4488]], [[0.4371]], [[0.4040]]], device=f'cuda:{self.gpu}')
        ...
        x = self.tail(res)
        if not self.synchronize_norm:
            x = x + torch.tensor([[[0.4488]], [[0.4371]], [[0.4040]]], device=f'cuda:{self.gpu}') # denormalization
        return x
