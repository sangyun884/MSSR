...
class PSNR(object):
    def __init__(self, device, val_max=1, val_min=0, ycbcr=True):
    ...
    def __call__(self,x,y):
        ...
        # Quantization
        if x.shape[0] == 1:
            x_pil = transforms.functional.to_pil_image(x[0].detach().cpu())
            x = transforms.functional.to_tensor(x_pil) * 255
            
            y_pil = transforms.functional.to_pil_image(y[0].detach().cpu())
            y = transforms.functional.to_tensor(y_pil) * 255
        ...
        with torch.no_grad():
            ...
            # psnr = 20*torch.log10(torch.tensor(self.val_max-self.val_min, dtype=torch.float, device=self.device)) - 10*torch.log10(mse)
